...
<cfcomponent hint="The CFCParser component provides methods to parse a ColdFusion Component.">
	...
	<cffunction name="findTags" access="private" returntype="array" output="false" hint="The findTags method searches the document for the given startTag and endTag. It returns an array of structures containing the locations in the document of the start and end position of each tag, and the full contents of the tag itself.">
	...
	<cffunction name="getFileHeader" access="private" output="false" returntype="string" hint="return any file header comments above the starting cfcomponent tag">
		<cfargument name="document" type="string" required="yes">
		<cfscript>
			var fileHeader = "";
			var startComponentTagPos = 0;

			startComponentTagPos = findNoCase("<cfcomponent",  arguments.document ,  0);
			if (startComponentTagPos GT 10){
				fileHeader = trim(left(arguments.document,  startComponentTagPos - 1));
			}
	...
	<cffunction name="parse" access="public" returntype="struct" output="false" hint="Provides the public interface to the CFC parser. This method should be passed the contents of a full ColdFusion component file.">
		<cfargument name="document" type="string" required="true" />
		<cfargument name="parseHeaderMode" type="string" required="true" />
		...
		<cfset var headerMetaData = "" />
		<cfset var fileHeader = "" />


		<cfswitch expression="#trim(ucase(arguments.parseHeaderMode))#">
			<cfcase value="javadoc">
				<cf_commentParser filecontents="#arguments.document#" r_stUDF="headerMetaData">
			</cfcase>
			<cfcase value="custom">
				<cfset fileHeader = getFileHeader(arguments.document) />
			</cfcase>
			<cfdefaultcase>
				<!--- do nothing --->
			</cfdefaultcase>
		...
		<cfset stComponent.attributes.headerMetaData = headerMetaData />
		<cfset stComponent.attributes.fileHeader = fileHeader />
